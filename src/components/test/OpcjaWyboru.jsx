...
class OpcjaWyboru extends PureComponent {
  ...
  render(){
    ...
    return ( 
        <button className={ btnStyle + " m-1 btn btn-sm btn-block text-left"} onClick={ () => onZaznaczOpcję(opcja) }>
          <span className="test__zadanie_id-pytania">{this.props.opcja.id}</span>{this.props.opcja.tresc}
        </button>
     );
  }
}
