...
class ModyfikowanieTestu extends PureComponent {
    ...
    onUsuńTest = async test => {
        try {
            const tekst2 = zdobądźTekstyWersjiJęzykowej("panelKlienta.trescGłówna.PanelKlientaTesty");
            ...
            NotificationManager.success(tekst2.notyfikacje.usuwanie.sukces);
                ...
                return (
                    <div className='react-confirm-alert__body'>
                        ...
                        <div className="d-flex">
                            <MDBBtn className="JW_RWD-button-sm" onClick={onClose} color="danger" size="sm">{this.tekst.confirmAlertTest.nie}</MDBBtn>
                            <MDBBtn className="JW_RWD-button-sm" onClick={() => { this.onUsuńTest(test); onClose() }} color="success" size="sm">{this.tekst.confirmAlertTest.tak}</MDBBtn>
                        </div>
                    </div>
                );
            }
    ...
    render() {
        ...
        return (
            <main className="JW_RWD-p-bigger text-left p-3">
                <hr className="mt-0" />
                <div className="d-flex flex-column flex-sm-row justify-content-between text-center">
                    <h5 className="font-weight-bold text-center mb-1 mb-md-3">{this.tekst.tytuł}</h5>
                    <div className="d-flex justify-content-between">
                        <MDBBtn className="JW_RWD-button-sm text-nowrap" color="danger" size="sm" outline onClick={() => this.uruchomOstrzeżenieDlaTestu(test)} >{this.tekst.przycisk.usuńTest}</MDBBtn>
                        <MDBBtn className="JW_RWD-button-sm text-nowrap" color="danger" size="sm" disabled={dokonanoZmian} onClick={this.początkoweUstawienia}>{this.tekst.przycisk.resetuj}</MDBBtn>
                        <MDBBtn className="JW_RWD-button-sm text-nowrap" color="success" size="sm" disabled={dokonanoZmian} onClick={this.sprawdźDaneIWyślijDoZapisu}>{this.tekst.przycisk.zatwierdźZmiany}</MDBBtn>
                    </div>
                </div>

                <span className="font-weight-bold">{this.tekst.opis}</span>
                <MDBInput label={this.tekst.inputy.labelNazwaTestu} autoComplete="off" value={test.nazwa} className="JW_RWD-p-bigger mr-5" name="nazwa" onChange={this.wprowadzanieZmian} />
                <MDBInput label={this.tekst.inputy.labelKrótkiOpis} autoComplete="off" value={test.krotkiOpis} className="JW_RWD-p-bigger mr-5" name="krotkiOpis" onChange={this.wprowadzanieZmian} />
                ...
                <div className="mb-3 m-0 p-0">
                    <div className="custom-control custom-checkbox">
                        <input name="czyLimit" type="checkbox" checked={test.limitCzasowy.czyLimit} className="JW_RWD-p-bigger custom-control-input" id={test.nazwa + "checkbox1"} onChange={this.wprowadzanieZmian} />
                    ...
                    {this.state.daneTestu.limitCzasowy.czyLimit &&
                        <div className="animated fadeIn">
                            <MDBInput name="minuty" type="number"
                                label={this.tekst.limitCzasowy.label.minuty}
                                autoComplete="off"
                                value={test.limitCzasowy.minuty}
                                className="JW_RWD-p-bigger m-0"
                                onChange={this.wprowadzanieZmian}
                            />
                            <MDBInput name="sekundy" type="number"
                                label={this.tekst.limitCzasowy.label.sekundy}
                                autoComplete="off"
                                value={test.limitCzasowy.sekundy}
                                className="JW_RWD-p-bigger m-0"
                                onChange={this.wprowadzanieZmian}
                            />
                        </div>
                ...
                <MDBInput name="instrukcja"
                    type="textarea"
                    className="JW_RWD-p-bigger pb-0"
                    label={this.tekst.instrukcja.inputLabel}
                    autoComplete="off"
                    rows='10'
                    value={test.instrukcja}
                    onChange={this.wprowadzanieZmian}
                />
